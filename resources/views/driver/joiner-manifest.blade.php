...
<head>
    ...
    <title>Joiner Trip Manifest | Driver Portal</title>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Inter',sans-serif; }
        ...
        .brand { font-size:16px; font-weight:700; color:#60a5fa; display:flex; align-items:center; gap:8px; }
        ...
        .step-circle {
            ...
            font-size:18px; margin-bottom:8px; background:white; color:#94a3b8;
        }
        .step-circle.active { border-color:#2563eb; color:#2563eb; background:#eff6ff; }
        .step-circle.done   { border-color:#2563eb; background:#2563eb; color:white; }
        ...
        .btn-arrived    { background:#2563eb; color:white; }
        ...
        .pay-badge {
            padding:4px 12px; border-radius:50px; font-size:11px; font-weight:700; text-transform:uppercase;
        }
        ...
        .collect-btn {
            ...
            background:#f59e0b; color:white; font-size:13px; font-weight:700;
...
<nav class="navbar">
    ...
    <span class="brand"><i class="fas fa-users"></i> Joiner Trip Manifest</span>
...
<div class="main">
    ...
    {{-- Trip Info --}}
    <div class="trip-card">
        <div class="trip-title"><i class="fas fa-map-marker-alt" style="margin-right:8px;"></i>{{ $trip->destination }}</div>
        <div class="trip-meta">
            <span><i class="fas fa-calendar"></i> {{ date('M d', strtotime($trip->trip_date)) }} – {{ $trip->end_date ? date('M d', strtotime($trip->end_date)) : 'TBD' }}</span>
            <span><i class="fas fa-map-pin"></i> Meet: {{ $trip->meetup_point }}</span>
            <span><i class="fas fa-bus"></i> {{ $trip->van }} ({{ $trip->plate_number }})</span>
            <span><i class="fas fa-users"></i> {{ $totalPassengers }} / {{ $trip->total_seats }} passengers</span>
        </div>
    </div>
    ...
    <div class="status-flow">
        ...
        @if($ts === 'not_started')
            <form method="POST" action="{{ route('driver.joiner.status', $trip->id) }}">
                ...
                <button type="submit" class="action-btn btn-arrived"
                        onclick="return confirm('Confirm: You have arrived at the meetup point?')">
        ...
        @elseif($ts === 'arrived')
            <form method="POST" action="{{ route('driver.joiner.status', $trip->id) }}">
                @csrf
                <input type="hidden" name="trip_status" value="in_progress">
                <button type="submit" class="action-btn btn-inprogress"
                        onclick="return confirm('Confirm: All passengers are on board?')">
            ...
            @if($allPaid)
                <form method="POST" action="{{ route('driver.joiner.status', $trip->id) }}">
                    ...
                    <button type="submit" class="action-btn btn-complete"
                            onclick="return confirm('Confirm: Trip completed and all payments collected?')">
    ...
    @elseif($trip->status === 'pending')
        <div style="background:#fef3c7;color:#92400e;border:1px solid #fcd34d;padding:14px 18px;border-radius:10px;margin-bottom:20px;font-weight:600;">
            <i class="fas fa-clock"></i> This trip is awaiting admin approval. You cannot start it yet.
        </div>
    @endif
    ...
    @forelse($bookings as $b)
        @php
            ...
            $badgeText  = $isPaid ? 'Fully Paid' : ($b->actual_paid > 0 ? 'Downpayment Paid' : 'Unpaid');
        ...
        <div class="pax-card {{ $isPaid ? 'paid-full' : 'not-paid' }}">
            <div class="pax-top">
                <div style="flex:1;">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;flex-wrap:wrap;">
                        <div class="pax-name" style="font-size:14px;min-width:120px;">{{ $pax->name }}</div>
                        <div class="pax-contact">
                            <i class="fas fa-phone" style="color:#60a5fa;font-size:10px;"></i>
                            {{ $pax->contact }}
                        </div>
            ...
            <div class="pax-amounts">
                <div class="amt-cell">
                    ...
                    <div class="amt-val">₱{{ number_format($b->total_price, 2) }}</div>
                </div>
                <div class="amt-cell">
            ...
            @if(!$isPaid && $trip->trip_status === 'in_progress')
                <form method="POST" action="{{ route('driver.joiner.collect', [$trip->id, $b->id]) }}">
                    @csrf
            ...
            @elseif($isPaid && $b->amount_paid > 0)
                {{-- Driver physically collected the cash --}}
                <div class="collected-msg"><i class="fas fa-check-circle"></i> Payment Collected</div>
            @elseif($isPaid)
                {{-- Paid online via Paymongo — driver did not collect --}}
    ...
    @empty
        <div style="background:white;border-radius:12px;padding:40px;text-align:center;color:#94a3b8;">
            ...
            No passengers yet for this trip.
        </div>
    @endforelse
    ...
    @if($allPaid && $bookings->count() > 0)
        <div style="background:#dcfce7;border:2px solid #bbf7d0;border-radius:14px;padding:20px 24px;margin-top:16px;text-align:center;">
            ...
            @if($totalCashCollected > 0)
                ...
                @if($totalOnlinePaid > 0)
                    <div style="font-size:12px;color:#1d4ed8;margin-top:4px;"><i class="fas fa-globe"></i> + ₱{{ number_format($totalOnlinePaid, 2) }} paid online</div>
                @endif
            @else
